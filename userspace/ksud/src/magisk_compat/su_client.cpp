...
int main(int argc, char** argv, char** envp) {
    ...
    if (sock < 0) {
        fprintf(stderr, "su: authorization daemon unavailable\n");
        return 1;
    }
    ...
    uint32_t na = 0;
    uint32_t ne = 0;
    const std::string payload = build_payload(cwd ? cwd : "/", argc, argv, envp, &na, &ne);
    if (payload.size() > kSuMaxPayload) {
        fprintf(stderr, "su: request too large\n");
        return 1;
    }
    ...
    if (!send_with_fds(sock, &hdr, sizeof(hdr), fds.data(), static_cast<int>(fds.size())) ||
    ...
    if (!read_all(sock, &res, sizeof(res)) || res.magic != kSuMagic) {
        fprintf(stderr, "su: no response from authorization daemon\n");
        return 1;
    }
    if (res.granted == 0) {
        fprintf(stderr, "su: access denied\n");
        return 1;
    }
