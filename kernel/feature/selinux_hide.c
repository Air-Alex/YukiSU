...
static void (*security_dump_masked_av_fn)(struct policydb *policydb,
					  struct context *scontext,
					  struct context *tcontext, u16 tclass,
					  u32 permissions,
					  const char *reason) = NULL;
static void (*context_struct_compute_av_fn)(
    struct policydb *policydb, struct context *scontext,
...
static ssize_t my_write_context(struct file *file, char *buf, size_t size)
{
	...
	if (length)
		goto out;
	length = security_context_to_sid_with_policy(
	    backup_sepolicy, buf, size, &sid, SECSID_NULL, GFP_KERNEL);
	if (length)
		goto out;

	length = security_sid_to_context_with_policy(backup_sepolicy, sid,
						     &canon, &len);
	if (length)
		goto out;
...
#else
	length =
	    avc_has_perm(&selinux_state, current_sid(), SECINITSID_SECURITY,
			 SECCLASS_SECURITY, SECURITY__CHECK_CONTEXT, NULL);
	if (length)
		goto out;

	length =
	    security_context_to_sid(&fake_state, buf, size, &sid, GFP_KERNEL);
	if (length)
		goto out;
...
static ssize_t my_write_access(struct file *file, char *buf, size_t size)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0)
	length = security_context_to_sid_with_policy(backup_sepolicy, scon,
						     strlen(scon), &ssid,
						     SECSID_NULL, GFP_KERNEL);
	if (length)
		goto out;

	length = security_context_to_sid_with_policy(backup_sepolicy, tcon,
						     strlen(tcon), &tsid,
						     SECSID_NULL, GFP_KERNEL);
	if (length)
		goto out;

	security_compute_av_user_with_policy(backup_sepolicy, ssid, tsid,
					     tclass, &avd);
#else
	length =
	    security_context_str_to_sid(&fake_state, scon, &ssid, GFP_KERNEL);
	if (length)
		goto out;

	length =
	    security_context_str_to_sid(&fake_state, tcon, &tsid, GFP_KERNEL);
	if (length)
		goto out;
	...
	// Stock reads 1; a loader load_policy may have bumped the backup before
	// we load.
	avd.seqno = 1;
...
static int __nocfi my_setprocattr(const char *name, void *value, size_t size)
{
	...
	if (size && str[0] && str[0] != '\n') {
		...
		error = security_context_to_sid_with_policy(
		    backup_sepolicy, str, size, &sid, SECSID_NULL, GFP_KERNEL);
#else
...
static void initialize_fake_status(void)
{
	...
	if (ksu_late_loaded) {
		// In late-load mode the captured page may already be modified.
	...
	pr_info("initialize_fake_status initialized: sequence=%d, "
		"policyload=%d, enforcing=%d\n",
...
typedef int (*sel_open_handle_status_fn)(struct inode *inode,
					 struct file *filp);
static sel_open_handle_status_fn orig_sel_open_handle_status;
...
static int my_sel_open_handle_status(struct inode *inode, struct file *filp)
{
	...
	ret = orig_sel_open_handle_status(inode, filp);
...
static int ksu_selinux_hide_enable()
{
	...
	if (!backup_sepolicy) {
		pr_err("no backup sepolicy available, please save feature and "
		       "reboot to retry!\n");
		return -EAGAIN;
	}
	...
	security_dump_masked_av_fn =
	    (void *)find_kernel_symbol_exact("security_dump_masked_av");
	if (!security_dump_masked_av_fn) {
		pr_warn("security_dump_masked_av not found!\n");
	}
	context_struct_compute_av_fn =
	    (void *)find_kernel_symbol_exact("context_struct_compute_av");
	...
	ret = ksu_patch_text(context_write, &my, sizeof(my),
	...
	ret = ksu_patch_text(access_write, &my, sizeof(my),
			     KSU_PATCH_TEXT_FLUSH_DCACHE);
	if (ret) {
...
static void ksu_selinux_hide_unhook()
{
	...
	if (orig_context_write) {
		ret = ksu_patch_text(context_write, &orig_context_write,
				     sizeof(orig_context_write),
				     KSU_PATCH_TEXT_FLUSH_DCACHE);
		orig_context_write = NULL;
		if (ret) {
			pr_err("selinux_hide: exit: patch_text context_write "
			       "err: %d\n",
			       ret);
		}
	}
	if (orig_access_write) {
		ret = ksu_patch_text(access_write, &orig_access_write,
				     sizeof(orig_access_write),
				     KSU_PATCH_TEXT_FLUSH_DCACHE);
		orig_access_write = NULL;
		if (ret) {
			pr_err("selinux_hide: exit: patch_text access_write "
			       "err: %d\n",
			       ret);
		}
	}
	if (sel_open_handle_status_slot && orig_sel_open_handle_status) {
		...
		if (ret) {
			pr_err("selinux_hide: exit: patch_text "
			       "sel_open_handle_status err: %d\n",
			       ret);
		}
	}
...
void ksu_selinux_hide_handle_post_fs_data(void)
{
	static_branch_disable(&fake_status_initialize_key);
	if (!fake_status)
		pr_err("selinux_hide: fake status is not initialized after "
		       "post-fs-data!\n");
}

static void hook_selinux_status_open(void)
{
	sel_open_handle_status_fn new_fn;
	...
	if (!sel_open_handle_status_slot) {
		ops = (struct file_operations *)find_kernel_symbol_exact(
		    "sel_handle_status_ops");
		if (!ops) {
			pr_err("selinux_hide: sel_handle_status_ops not found, "
			       "fake status will not work\n");
			return;
		}
		sel_open_handle_status_slot = &ops->open;
	}

	new_fn = my_sel_open_handle_status;
	orig_sel_open_handle_status = *sel_open_handle_status_slot;
	ret = ksu_patch_text(sel_open_handle_status_slot, &new_fn,
			     sizeof(new_fn), KSU_PATCH_TEXT_FLUSH_DCACHE);
	if (ret) {
		pr_err("selinux_hide: init: patch_text sel_open_handle_status "
		       "err: %d\n",
		       ret);
...
static int string_to_context_struct(struct policydb *pol,
				    struct sidtab *sidtabp, char *scontext,
					       ...
					       u32 def_sid, gfp_t gfp_flags)
{
	...
	*sid = SECSID_NULL;
	...
	rc = string_to_context_struct(policydb, sidtab, scontext2, &context,
				      def_sid);
	if (rc)
		goto out;
 ...
 */
static int context_struct_to_string(struct policydb *p, struct context *context,
...
static int sidtab_entry_to_string(struct policydb *p, struct sidtab *sidtab,
				  struct sidtab_entry *entry, char **scontext,
				  u32 *scontext_len)
{
 ...
 */
static void __nocfi type_attribute_bounds_av(struct policydb *policydb,
					     struct context *scontext,
					     ...
					     struct av_decision *avd)
{
	...
	context_struct_compute_av(policydb, &lo_scontext, tcontextp, tclass,
				  &lo_avd, NULL);
	...
	if (security_dump_masked_av_fn)
		security_dump_masked_av_fn(policydb, scontext, tcontext, tclass,
					   masked, "bounds");
}
...
static void __nocfi security_compute_av_user_with_policy(
    struct selinux_policy *policy, u32 ssid, u32 tsid, u16 tclass,
    struct av_decision *avd)
{
	struct policydb *policydb;
	struct sidtab *sidtab;
	...
	if (context_struct_compute_av_fn) {
		context_struct_compute_av_fn(policydb, scontext, tcontext,
					     tclass, avd, NULL);
	} else {
		context_struct_compute_av(policydb, scontext, tcontext, tclass,
					  avd, NULL);
	}
out:
	return;
