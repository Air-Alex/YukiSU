 ...
 * a negative dentry and readdir omits the name for view-target observers, so
 * the real entry disappears for the view without a per-syscall TSR route.
 * Non-target observers keep resolving the real entry.  Sleepable context only.
 */
