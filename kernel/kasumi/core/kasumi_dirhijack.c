...
static DECLARE_WORK(kasumi_dh_reap_dops_work, kasumi_dh_reap_dops_workfn);
...
{
	return READ_ONCE(kasumi_dh_ready);
}
...
kasumi_dirhijack_original_iops(const struct inode *inode)
{
	...
	struct kasumi_dh_iop *meta = kasumi_dh_iop_of(inode);
...
static bool kasumi_dh_current_sees(void)
{
	if (!kasumi_dirhijack_enabled())
		return false;
	/* Sole path-view engine, so this is the only serve gate: an
	 * unregistered rule simply has no child here and falls through to the
	 * real entry. */
							  ...
							  unsigned int flags)
{
	...
	if (c && ((c->flags & KASUMI_VNODE_F_SU)
		...
		v_ino = c->v_ino;
		cflags = c->flags;
		is_hide = c->hide || ((c->flags & KASUMI_VNODE_F_SU) &&
...
static int KASUMI_NOCFI kasumi_dh_iterate_inner(
    struct file *file, struct dir_context *ctx,
    const struct file_operations *orig, struct kasumi_dh_dir *dn)
{
	...
	if (!orig || !orig->iterate_shared)
		return -ENOTDIR;
	if (!dn || (!kasumi_dh_current_sees() && !ksu_sucompat_vfs_enabled()))
...
kasumi_dh_dop_lookup_locked(const struct dentry *dentry)
{
	...
	hash_for_each_possible(kasumi_dh_dops, m, node, (unsigned long)dentry)
	{
		if (m->dentry == dentry)
			return m;
	}
...
static int KASUMI_NOCFI kasumi_dh_revalidate(struct inode *dir,
...
#else
static int KASUMI_NOCFI kasumi_dh_revalidate(struct dentry *dentry,
					     unsigned int flags)
{
	struct kasumi_dh_dop_meta *dm;
					...
					struct list_head *retired)
{
	...
	if (dentry->d_op == &m->shadow_dop) {
		...
		WRITE_ONCE(dentry->d_flags,
			   (flags & ~KASUMI_DH_DOP_FLAGS) | m->orig_op_flags);
 ...
 */
void KASUMI_NOCFI kasumi_dh_reclaim_destroy_inode(
...
void KASUMI_NOCFI kasumi_dh_reclaim_evict_inode(
    struct inode *inode, const struct super_operations *orig)
{
	if (kasumi_vnode_is_ours(inode)) {
...
int KASUMI_NOCFI kasumi_dh_reclaim_drop_inode(
    struct inode *inode, const struct super_operations *orig)
{
	if (kasumi_vnode_is_ours(inode))
...
{
	struct kasumi_dh_child *c =
	    container_of(rcu, struct kasumi_dh_child, rcu);
				   ...
				   bool lookup_only, const char *vpath)
{
	...
	if (old && (old->flags & KASUMI_VNODE_F_SU) &&
...
static KASUMI_NOCFI int kasumi_dh_register_vtopo(const char *visible_path)
{
	...
	ret = kasumi_dh_dir_add_child(
	    dir, seg, (u16)seg_len, NULL, kasumi_vnode_vpath_ino(vpath),
	    KASUMI_VNODE_F_DIR | KASUMI_VNODE_F_VIRTUAL_DIR, false, false,
	    vpath);
	if (ret) {
		if (sop_registered) {
 ...
 * negative dentry (-ENOENT) and readdir omits the name for view-target
 * observers, while non-target observers keep resolving the real entry.  Sinks a
 * hide rule off the TSR path routes.  Sleepable context only.  Returns 0 or a
 * negative errno.
 */
...
void kasumi_dirhijack_clear(void)
{
	...
	list_for_each_entry_safe (dir, dtmp, &kasumi_dh_dirs, list) {
		...
		if (READ_ONCE(kasumi_dh_ready)) {
			hlist_for_each_entry(child, &dir->children, node)
			    reserved |= !!(child->flags & KASUMI_VNODE_F_SU);
		}
		if (reserved) {
			hlist_for_each_entry_safe(child, next, &dir->children,
						  node)
			{
				if (child->flags & KASUMI_VNODE_F_SU)
					continue;
				hlist_del_rcu(&child->node);
				call_rcu(&child->rcu, kasumi_dh_child_free_rcu);
			}
			continue;
		}
	...
	{
		struct kasumi_dh_child *child =
		    dm->dir ? kasumi_dh_find_child(dm->dir, dm->name,
						   dm->name_len, dm->name_hash)
			    : NULL;
		if (READ_ONCE(kasumi_dh_ready) && child &&
		    (child->flags & KASUMI_VNODE_F_SU))
			continue;
		kasumi_dh_retire_dop_locked(dm, &retired_dops);
	}
	mutex_unlock(&kasumi_dh_lock);
...
int kasumi_dirhijack_init(void)
{
	...
	pr_info("kasumi: dirhijack initialized\n");
	return 0;
}
