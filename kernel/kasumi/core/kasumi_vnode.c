...
bool kasumi_vnode_is_ours(const struct inode *inode)
{
	...
	if (!inode)
		return false;
	iop = kasumi_dirhijack_original_iops(inode);
	return iop == &kasumi_vnode_file_iops ||
	       iop == &kasumi_vnode_dir_iops ||
...
kasumi_vnode_getattr(KVN_IDMAP_ARG const struct path *path, struct kstat *stat,
 ...
 */
static int KASUMI_NOCFI
kasumi_vnode_setattr(KVN_IDMAP_ARG struct dentry *dentry, struct iattr *attr)
{
...
static int KASUMI_NOCFI kasumi_vnode_open(struct inode *inode,
					  struct file *file)
{
...
static ssize_t
    KASUMI_NOCFI kasumi_vnode_splice_read(struct file *in, loff_t *ppos,
 ...
 * (kasumi_vnode_open) then opens the *real* source via dentry_open, which
...
static __poll_t KASUMI_NOCFI
kasumi_vnode_special_poll(struct file *file, struct poll_table_struct *wait)
{
	struct file *real = file->private_data;
...
static int KASUMI_NOCFI kasumi_vnode_dir_iterate_virtual(
    struct file *file, struct dir_context *ctx, struct kasumi_vnode_info *info)
{
	...
	if (info->source.dentry && info->source.mnt) {
		...
		if (ci) {
			u8 cf = S_ISDIR(ci->i_mode)   ? KASUMI_VNODE_F_DIR
				: S_ISLNK(ci->i_mode) ? KASUMI_VNODE_F_LNK
						      : 0;

			cvi = kasumi_vnode_new(
			    dir->i_sb, &child,
		...
		int kind = kasumi_rule_vpath_child(info->visible_path, name,
						   &leaf, &lmode, &lino);

		if (kind == KASUMI_VPATH_LEAF) {
			u8 cf = S_ISDIR(lmode)	 ? KASUMI_VNODE_F_DIR
				: S_ISLNK(lmode) ? KASUMI_VNODE_F_LNK
						 : 0;

			cvi =
			    kasumi_vnode_new(dir->i_sb, &leaf, lino, lmode, cf);
			path_put(&leaf);
			...
			if (cvp) {
				cvi = kasumi_vnode_new_virtual(
				    dir->i_sb, cvp, kasumi_vnode_vpath_ino(cvp),
				    dir);
				kfree(cvp);
			}
		}
...
kasumi_vnode_src_child(struct kasumi_vnode_info *info, const struct dentry *vis,
 ...
 */
static int KASUMI_NOCFI kasumi_vnode_publish_child(
    struct super_block *sb, const struct path *src_parent,
    struct dentry *src_child, struct dentry *vis)
{
	...
	cvi = kasumi_vnode_new(
	    sb, &cpath,
	    kasumi_vnode_source_ino(ci->i_sb ? ci->i_sb->s_dev : 0,
				    (u64)ci->i_ino),
	    ci->i_mode, cf);
	if (!cvi)
		return -ENOMEM;
						      ...
						      umode_t mode)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 18, 0)
	{
		...
		if (IS_ERR(created)) {
			...
			child = created;
			ret = kasumi_vnode_publish_child(
			    dir->i_sb, &info->source, child, dentry);
		}
	}
#else
...
static struct dentry *
    KASUMI_NOCFI kasumi_vnode_dir_mkdir(KVN_IDMAP_ARG struct inode *dir,
...
static int KASUMI_NOCFI kasumi_vnode_dir_symlink(
    KVN_IDMAP_ARG struct inode *dir, struct dentry *dentry, const char *oldname)
{
	struct kasumi_vnode_info *info = dir->i_private;
	struct inode *src_dir = NULL;
	struct dentry *child;
					      ...
					      struct dentry *dentry)
{
	...
	struct kasumi_vnode_info *old_info =
	    (old_vi && kasumi_vnode_is_ours(old_vi)) ? old_vi->i_private : NULL;
	struct inode *src_dir = NULL;
	struct dentry *child;
	int ret;
...
static int KASUMI_NOCFI kasumi_vnode_dir_rename(
    KVN_IDMAP_ARG struct inode *old_dir, struct dentry *old_dentry,
...
static KASUMI_NOCFI void kasumi_vnode_clone_sid(struct inode *vnode,
						struct inode *src)
{
	if (!vnode || !src || !kasumi_security_inode_getsecctx ||
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 14, 0)
	{
		...
		int ret = kasumi_security_inode_getsecctx(src, &ctx);

		if (!ret && ctx.context && ctx.len)
			(void)kasumi_security_inode_notifysecctx(
			    vnode, ctx.context, ctx.len);
		if (ctx.context)
			kasumi_security_release_secctx(&ctx);
	}
#else
	{
		void *ctx = NULL;
		u32 ctxlen = 0;
		int ret = kasumi_security_inode_getsecctx(src, &ctx, &ctxlen);

		if (!ret && ctx && ctxlen)
			(void)kasumi_security_inode_notifysecctx(vnode, ctx,
								 ctxlen);
		if (ctx)
...
struct inode *kasumi_vnode_new(struct super_block *sb,
			       const struct path *source, unsigned long v_ino,
			       umode_t mode, u8 flags)
{
	struct inode *inode;
	...
	if ((flags & KASUMI_VNODE_F_SU) &&
	    ksu_sucompat_vfs_setup_inode(inode)) {
		iput(inode);
		return NULL;
	}
	if (r_inode)
