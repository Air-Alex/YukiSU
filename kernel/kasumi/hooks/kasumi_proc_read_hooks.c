...
static int kasumi_mount_hide_pre(struct kprobe *p, struct pt_regs *regs)
{
	...
	if (!sb || !kasumi_valid_kernel_addr((unsigned long)sb))
		return 0;
	fstype = sb->s_type;
	if (!fstype || !kasumi_valid_kernel_addr((unsigned long)fstype) ||
	    !fstype->name)
		return 0;
...
typedef int (*kasumi_proc_mount_show_fn)(struct seq_file *, struct vfsmount *);
...
struct kasumi_mount_file_proxy {
	...
	bool stream_failed;
	bool
	    stream_produced; /* whole-file (MOUNTINFO/MOUNTS) view built once */
};
	...
	for (;;) {
		...
		if (complete_len > 0) {
			if (proxy->kind == KASUMI_PROC_PROXY_MAPS) {
				 ...
				 * kasumi_mount_proxy_serve_whole(); only MAPS
				 * uses this incremental line-chunk path. */
				return -EIO;
			}
 ...
 */
static int kasumi_mount_proxy_slurp_orig(struct kasumi_mount_file_proxy *proxy,
					 struct file *file)
{
	struct kvec kvec;
	struct iov_iter kernel_iter;
...
static int kasumi_mount_proxy_reset_seq(struct kasumi_mount_file_proxy *proxy,
					struct file *file,
					kasumi_proc_mount_show_fn show)
{
	struct kasumi_proc_mounts_prefix *p;
...
static int kasumi_mount_proxy_restore_seq(struct kasumi_mount_file_proxy *proxy,
					  struct file *file,
					  kasumi_proc_mount_show_fn show)
{
	struct kasumi_proc_mounts_prefix *p;
...
static bool kasumi_mount_proxy_pair_mountpoints_match(const char *mountinfo,
						      size_t mountinfo_len,
						      const char *mounts,
						      size_t mounts_len)
{
	...
	while (mi_pos < mountinfo_len && mounts_pos < mounts_len) {
		...
		if (!kasumi_mount_proxy_line_token(
			mountinfo + mi_line_start, mi_pos - mi_line_start, 4,
			&mi_target, &mi_target_len) ||
		    !kasumi_mount_proxy_line_token(
			mounts + mounts_line_start,
			mounts_pos - mounts_line_start, 1, &mounts_target,
			&mounts_target_len) ||
...
static void
kasumi_canonical_observer_release(struct kasumi_canonical_observer_view *view)
{
	if (view->mountinfo) {
	...
	if (view->mounts) {
		kvfree(view->mounts);
		atomic_long_sub(view->mounts_len, &kasumi_stream_memory_used);
	}
	if (view->tgid)
		put_pid(view->tgid);
...
static int
kasumi_mount_proxy_canonicalize_observer(struct kasumi_mount_file_proxy *proxy,
					 size_t *mountinfo_len,
					 size_t *mounts_len)
{
	...
	if (!proxy || !proxy->stream_filtered || !proxy->stream_raw ||
	    ...
	    !owner ||
	    check_add_overflow(*mountinfo_len, *mounts_len, &reserve_len))
		return -EINVAL;
 ...
 */
static int
kasumi_mount_proxy_produce_whole(struct kasumi_mount_file_proxy *proxy,
				 struct file *file)
{
	...
	if (proxy->kind == KASUMI_PROC_PROXY_MOUNTINFO ||
		...
		/* Fd installation captured the kernel's exact canonical
		 * callback without opening or reading a second proc file.  It
		 * must still be installed before the first lazy render starts.
		 */
		...
		for (attempt = 0; attempt < 3 && ret == -EAGAIN; attempt++) {
			...
			for (;;) {
				...
				if (ret != -ENOSPC)
					break;
				ret = kasumi_mount_proxy_stream_grow_filtered(
				    proxy);
				if (ret)
					break;
			}
			...
			pair_mountinfo =
			    kvmalloc(pair_mountinfo_len, GFP_KERNEL);
			if (!pair_mountinfo) {
				atomic_long_sub(pair_mountinfo_len,
						&kasumi_stream_memory_used);
				pair_mountinfo_len = 0;
			...
			memcpy(pair_mountinfo, proxy->stream_raw,
			       pair_mountinfo_len);
			...
			ret = kasumi_filter_mounts_by_ordinal(
			    proxy->stream_raw, proxy->stream_raw_len, visible,
			    built_lines, &mounts_len);
			if (!ret) {
				proxy->stream_raw_len = mounts_len;
				proxy->stream_out_len = mounts_len;
				ret = kasumi_fake_mi_publish_current_snapshot(
				    pair_mountinfo, pair_mountinfo_len,
				    proxy->stream_filtered, mi_len, pm->ns,
				    &pm->root, &published_generation);
			}
		}
		kvfree(visible);
		if (pair_mountinfo) {
		...
		if (ret)
			goto fail_restore;
		ret = kasumi_mount_proxy_canonicalize_observer(proxy, &mi_len,
							       &mounts_len);
		if (ret)
			goto fail_restore;
		proxy->stream_raw_len = mounts_len;
		proxy->stream_out_len =
		    proxy->kind == KASUMI_PROC_PROXY_MOUNTINFO ? mi_len
							       : mounts_len;
	} else {
		ret = -EIO;
		goto fail_restore;
	 ...
	 * Non-current open-bound views never publish, so retain the generation
	 * from before production and let poll expose any concurrent
	 * invalidation.
	 */
	WRITE_ONCE(proxy->stream_generation, published_generation
						 ? published_generation
						 : production_generation);
	/* Publish the completed bytes/generation before poll can treat this
	 * stream as produced. If invalidation woke waiters before this flag
	 * became visible, close that lost-wakeup window explicitly after
	 * publication.
	 */
	smp_store_release(&proxy->stream_produced, true);
	if (READ_ONCE(proxy->stream_generation) != kasumi_fake_mi_generation())
...
static ssize_t
kasumi_mount_proxy_serve_whole(struct kasumi_mount_file_proxy *proxy,
			       ...
			       struct iov_iter *to, size_t count, loff_t *ppos)
{
	...
	/* MOUNTINFO is built into stream_filtered; MOUNTS is filtered in place
	 * in stream_raw (the kernel's real /proc/mounts bytes, minus hidden
	 * lines). */
...
{
	struct kasumi_mount_file_proxy *proxy = container_of(
	    iocb->ki_filp->f_op, struct kasumi_mount_file_proxy, proxy_fops);
	ssize_t ret;
	int srcu_idx;
	...
	if (proxy->kind == KASUMI_PROC_PROXY_MOUNTINFO ||
		...
		ret = kasumi_mount_proxy_serve_whole(proxy, iocb->ki_filp, NULL,
						     ...
						     &iocb->ki_pos);
		kasumi_log(
		    "mount_proxy: fake_read_iter pid=%d comm=%s ret=%zd\n",
	...
	if (proxy->kind == KASUMI_PROC_PROXY_MAPS) {
		ret = kasumi_mount_proxy_filtered_read(
		    proxy, iocb->ki_filp, NULL, to, iov_iter_count(to),
		    &iocb->ki_pos);
		goto out;
	}
						     ...
						     loff_t offset, int whence)
{
	...
	if (whence == SEEK_SET && offset == 0) {
		ret = kasumi_mount_proxy_restore_seq(proxy, file,
						     proxy->orig_mount_show);
		if (ret)
			goto out;
		proxy->stream_raw_len = 0;
		proxy->stream_tail_off = 0;
...
kasumi_mount_proxy_poll(struct file *file, struct poll_table_struct *wait)
{
	...
	if (smp_load_acquire(&proxy->stream_produced) &&
	    READ_ONCE(proxy->stream_generation) != kasumi_fake_mi_generation())
					   ...
					   enum kasumi_policy_scope scope)
{
	...
	if (kind == KASUMI_PROC_PROXY_MOUNTINFO ||
	    kind == KASUMI_PROC_PROXY_MOUNTS) {
		/* Passive capture only: @file is the caller's completed proc
		 * open at fd installation.  Never open/read another proc file
		 * to discover this canonical CFI callback, which would advance
		 * observable timing.
		 */
		pm = kasumi_mount_proxy_proc_mounts(file);
		if (!pm)
	...
	if (kind == KASUMI_PROC_PROXY_MAPS)
		...
		WRITE_ONCE(file->f_mode, proxy->orig_f_mode & ~FMODE_PREAD);
...
static int kasumi_cp_statx_pre(struct kprobe *kp, struct pt_regs *regs)
{
	...
	if (result_mask & STATX_MNT_ID_UNIQUE) {
		...
		/* The unique ID cannot be checked against the namespace-local
		 * hidden map, so suppress the mount-root side channel
		 * unconditionally too. */
	...
	lookup_ret = kasumi_fake_mi_mount_id_state_cached(
	    real_mnt_id, &fake_mnt_id, &hidden, &projected_mount_root);
	if (!lookup_ret && hidden) {
		u64 attributes = READ_ONCE(stat->attributes);
		u64 attributes_mask = READ_ONCE(stat->attributes_mask);

		/* Removing an overmount exposes the visible ancestor. A real
		 * mount root remains a projected mount root only when both
		 * mounts begin at the same namespace path (recorded in the
		 * snapshot's id map).
		 */
		if (!(attributes & (u64)STATX_ATTR_MOUNT_ROOT) ||
		    !projected_mount_root)
			attributes &= ~(u64)STATX_ATTR_MOUNT_ROOT;
		WRITE_ONCE(stat->attributes, attributes);
		WRITE_ONCE(stat->attributes_mask,
			   attributes_mask | (u64)STATX_ATTR_MOUNT_ROOT);
	} else if (lookup_ret) {
		u64 attributes = READ_ONCE(stat->attributes);
		 ...
		 * mount-root bit is safe to expose. Publish an authoritative
		 * false value until a matching snapshot is prepared.
		 */
		WRITE_ONCE(stat->attributes,
			   attributes & ~(u64)STATX_ATTR_MOUNT_ROOT);
		WRITE_ONCE(stat->attributes_mask,
	...
	if (lookup_ret || fake_mnt_id <= 0) {
		/* cp_statx copies this kstat after the probe returns. Publish a
		 * safe result now; a later call can translate it after the
		 * mountinfo cache refreshes naturally.
		 */
		WRITE_ONCE(stat->mnt_id, 0);
...
void kasumi_proc_read_hooks_init(void)
{
	...
	unsigned long fd_install_addr = kasumi_lookup_name_quiet("fd_install");
	...
	unsigned long show_mountinfo_addr =
	    kasumi_lookup_name("show_mountinfo");
	 ...
	 * store them directly in proc_mounts->show on old jump-table CFI
	 * kernels.
	 */
	kasumi_show_vfsmnt_raw = (kasumi_proc_mount_show_fn)show_vfsmnt_addr;
	...
	if (cp_statx_addr) {
		...
		if (!register_kprobe(&kasumi_kp_cp_statx)) {
			...
			pr_info(
			    "kasumi: statx mount IDs projected via cp_statx\n");
		} else {
			pr_warn("kasumi: register_kprobe(cp_statx) failed\n");
		}
	} else {
		pr_warn("kasumi: cp_statx not found, statx mount ID spoof "
			"disabled\n");
	}
	...
	if (kasumi_show_vfsmnt_raw && kasumi_show_mountinfo_raw) {
		...
		if (fd_install_addr) {
			...
			kasumi_kp_fd_install.pre_handler =
			    kasumi_fd_install_pre;
		...
		pr_warn("kasumi: proc fd proxy disabled without both mount "
			"render callbacks\n");
	}
	...
	if (!use_proxy_filter) {
		if (show_vfsmnt_addr) {
			...
			if (register_kprobe(&kasumi_kp_show_vfsmnt) == 0) {
				...
				pr_info("kasumi: mount hide via kprobe on "
		...
		if (show_mountinfo_addr) {
			...
			if (register_kprobe(&kasumi_kp_show_mountinfo) == 0) {
				...
				pr_info(
				    "kasumi: mount hide via kprobe on "
				    "show_mountinfo (/proc/pid/mountinfo)\n");
			}
		} else {
			pr_warn("kasumi: show_mountinfo not found\n");
		}
		pr_warn(
		    "kasumi: maps spoof unavailable without per-open proxy\n");
	}
}
...
void kasumi_proc_read_hooks_exit(void)
{
	...
	/* Proxy fops pin THIS_MODULE, so final destruction starts only after
	 * all installed proc files have naturally reached ->release.
	 */
